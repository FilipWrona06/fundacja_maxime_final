...
// Importy z bibliotek zewnętrznych i Next.js
...
// Importy danych i typów z projektu
...
const useOnClickOutside = (refs: ReadonlyArray<RefObject<HTMLElement | null>>, handler: (event: MouseEvent | TouchEvent) => void) => {
...
// --- KOMPONENTY POMOCNICZE (SPECYFICZNE DLA NAVBARA) ---
...
const AnimatedMenuButton = memo(({ isOpen, onClick, buttonRef }: AnimatedMenuButtonProps) => (
    ...
    >
        <motion.div className="absolute inset-0 rounded-lg bg-arylideYellow/0" whileHover={{ backgroundColor: 'rgba(233,215,88,0.1)' }} transition={{ duration: 0.3 }} />
        <motion.div className="flex flex-col justify-around w-6 h-6 relative z-10" animate={isOpen ? 'open' : 'closed'} initial={false} transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}>
...
interface PatroniteLinkProps { isMobile?: boolean; onClick?: () => void; }
